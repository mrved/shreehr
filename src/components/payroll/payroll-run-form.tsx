...
export function PayrollRunForm() {
  ...
  async function handleValidate() {
    ...
    try {
      ...
      if (!lockData.data) {
        setError(
          `Attendance is not locked for ${MONTHS[month - 1].label} ${year}. Please lock attendance first.`,
        );
      ...
      const existing = runsData.data?.find(
        (r: any) => r.month === month && r.status !== "REVERTED",
      );

      if (existing) {
        setError(
          `Payroll for ${MONTHS[month - 1].label} ${year} already exists (${existing.status})`,
        );
        setValidationResult(null);
        return;
      }
  ...
  return (
    <Card>
      ...
      <CardContent className="space-y-4">
        ...
        <Button onClick={handleValidate} disabled={validating} variant="outline" className="w-full">
        ...
        <Button onClick={handleSubmit} disabled={loading || !validationResult} className="w-full">
