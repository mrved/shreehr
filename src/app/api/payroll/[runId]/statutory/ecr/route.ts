...
export async function GET(
  ...
  try {
    ...
    const ecrContent = await generateECRForPayrollRun(runId, establishmentCode, establishmentName);
    ...
    // Count records
    const recordCount = payrollRun.records.filter((r) => r.pf_employee_paise > 0).length;
    ...
    return NextResponse.json({ error: "Failed to generate ECR file" }, { status: 500 });
  }
}
