...
import { saveFile, validateFile, calculateRetentionDate } from '@/lib/storage';

export async function GET(
  request: NextRequest,
  { params }: { params: { declarationId: string } }
) {
...
export async function DELETE(request: NextRequest) {
  ...
  try {
    ...
    const isAdmin = ['ADMIN', 'SUPER_ADMIN', 'HR_MANAGER'].includes(
      session.user.role || ''
    );
