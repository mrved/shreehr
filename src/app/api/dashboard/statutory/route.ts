...
export async function GET(request: NextRequest) {
  ...
  const filedOnTime = recentDeadlinesAll.filter(
    (d) => d.status === "FILED" && d.filed_at && d.filed_at <= d.due_date,
  ).length;

  const totalApplicable = recentDeadlinesAll.filter((d) => d.status !== "NOT_APPLICABLE").length;

  const complianceScore =
    totalApplicable > 0 ? Math.round((filedOnTime / totalApplicable) * 100) : 100;

  return NextResponse.json({
    summary: {
      overdue: overdueDeadlines.length,
      dueSoon: upcomingDeadlines.filter((d) => differenceInDays(d.due_date, today) <= 3).length,
