...
const NEW_REGIME_SLABS = [
  ...
  { min: 700000, max: 1000000, rate: 0.1 }, // 7L - 10L - 10%
  { min: 1000000, max: 1200000, rate: 0.15 }, // 10L - 12L - 15%
  { min: 1200000, max: 1500000, rate: 0.2 }, // 12L - 15L - 20%
  { min: 1500000, max: Infinity, rate: 0.3 }, // Above 15L - 30%
];
...
const OLD_REGIME_SLABS = [
  ...
  { min: 1000000, max: Infinity, rate: 0.3 }, // Above 10L - 30%
];
...
export function calculateMonthlyTDS(
  ...
  const standardDeduction =
    regime === "NEW" ? STANDARD_DEDUCTION_NEW_REGIME : STANDARD_DEDUCTION_OLD_REGIME;
 ...
 */
function calculateTaxFromSlabs(incomeRupees: number, slabs: typeof NEW_REGIME_SLABS): number {
  let tax = 0;
  ...
  for (const slab of slabs) {
    ...
    const slabWidth =
      slab.max === Infinity ? remainingIncome : Math.min(remainingIncome, slab.max - slab.min);
