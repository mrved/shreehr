...
export const investment80CSchema = z.object({
  ...
  const total = data.section_80c_ppf +
...
export const investment80DSchema = z.object({
...
export const hraSchema = z.object({
  ...
  hra_landlord_pan: z.string().regex(PAN_REGEX, 'Invalid PAN format').optional().or(z.literal('')),
  ...
  if (annualRent > 10000000) { // Rs.1,00,000 = 10000000 paise
...
export const otherDeductionsSchema = z.object({
...
export const investmentCreateSchema = z.object({
  financial_year: z.string().regex(/^\d{4}-\d{2}$/, 'Financial year must be in format YYYY-YY (e.g., 2025-26)'),
...
export const investmentUpdateSchema = investmentCreateSchema.partial().extend({
  financial_year: z.string().regex(/^\d{4}-\d{2}$/, 'Financial year must be in format YYYY-YY (e.g., 2025-26)').optional(),
});
...
export function calculate80CTotal(declaration: any): number {
  return (declaration.section_80c_ppf || 0) +
...
export function calculate80DTotal(declaration: any): number {
  return (declaration.section_80d_self || 0) +
